    ...
    <SafeAreaView style={styles.page}><StatusBar style="dark" />
      ...
      <ScrollView contentContainerStyle={styles.detailBody}>
        ...
        <View style={styles.infoGrid}>
          {[['Platform', item.platform], ['Track', item.track]].map(([label, value]) => <View style={styles.infoCell} key={label}><Text style={styles.infoLabel}>{label}</Text><Text style={styles.infoValue}>{value || "—"}</Text></View>)}
        </View>
        {legs.length > 1 && <View style={styles.detailSection}><Text style={styles.sectionTitle}>Journey legs</Text>{legs.map((leg, index) => <View key={`${leg.trainNumber}-${index}`}><View style={styles.legCard}><View style={styles.legTop}><View style={styles.trainPill}><Ionicons name="train-outline" size={14} color={green} /><Text style={styles.trainPillText}>{leg.trainNumber || `Leg ${index + 1}`}</Text></View>{leg.operator && <Text style={styles.legOperator}>{leg.operator}</Text>}</View><View style={styles.legRoute}><View style={styles.grow}><Text style={styles.legTime}>{leg.departureAt ? formatTime(new Date(leg.departureAt)) : "—"}</Text><Text style={styles.legStation}>{leg.origin || "Origin"}</Text></View><Ionicons name="arrow-forward" size={18} color="#929A94" /><View style={[styles.grow, styles.alignRight]}><Text style={styles.legTime}>{leg.arrivalAt ? formatTime(new Date(leg.arrivalAt)) : "—"}</Text><Text style={[styles.legStation, styles.textRight]}>{leg.destination || "Destination"}</Text></View></View>{(leg.platform || leg.track) && <Text style={styles.legPlatform}>{leg.platform ? `Platform ${leg.platform}` : ""}{leg.platform && leg.track ? "  ·  " : ""}{leg.track ? `Track ${leg.track}` : ""}</Text>}</View>{index < legs.length - 1 && <View style={styles.changeRow}><View style={styles.changeLine} /><View style={styles.changePill}><Ionicons name="swap-horizontal" size={14} color="#6C746E" /><Text style={styles.changeText}>Change at {leg.destination}</Text></View><View style={styles.changeLine} /></View>}</View>)}</View>}
        {passengers.length > 0 && <View style={styles.detailSection}><Text style={styles.sectionTitle}>Seats by train</Text>{passengers.map((passenger, index) => <View style={styles.passengerCard} key={`${passenger.name}-${index}`}><View style={styles.passengerHead}><View style={styles.passengerIcon}><Ionicons name="person" size={16} color={green} /></View><Text style={styles.passengerName}>{passenger.name || `Passenger ${index + 1}`}</Text></View><View style={styles.reservations}>{passenger.seats?.map((reservation, seatIndex) => <View style={styles.reservation} key={`${reservation.trainNumber}-${seatIndex}`}><Text style={styles.reservationTrain}>{reservation.trainNumber || "Train"}</Text><Text style={styles.reservationText}>Coach <Text style={styles.metaStrong}>{reservation.carriage || "—"}</Text></Text><Text style={styles.reservationText}>Seat <Text style={styles.metaStrong}>{reservation.seat || "—"}</Text></Text></View>)}</View></View>)}</View>}
...
const styles = StyleSheet.create({
  ...
  dayDivider: { flexDirection: "row", alignItems: "center", gap: 13, marginTop: 3, marginBottom: 12 }, dayTitle: { color: ink, fontSize: 14, fontWeight: "800" }, dayLine: { flex: 1, height: 1, backgroundColor: "#D6D9D3" },
  ...
  detailSection: { marginTop: 24, gap: 12 }, legCard: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#E2E4DE", borderRadius: 16, padding: 16 }, legTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }, legOperator: { color: "#767D78", fontSize: 11 }, legRoute: { flexDirection: "row", alignItems: "center", gap: 12 }, legTime: { color: ink, fontSize: 19, fontWeight: "800" }, legStation: { color: "#59615C", fontSize: 13, fontWeight: "600", marginTop: 3 }, legPlatform: { color: green, fontSize: 12, fontWeight: "700", marginTop: 14, paddingTop: 12, borderTopWidth: 1, borderColor: "#ECEDE8" }, changeRow: { flexDirection: "row", alignItems: "center", marginVertical: 7 }, changeLine: { height: 1, flex: 1, backgroundColor: "#D5D9D3" }, changePill: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9 }, changeText: { color: "#6C746E", fontSize: 11, fontWeight: "700" }, passengerCard: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#E2E4DE", borderRadius: 16, padding: 16 }, passengerHead: { flexDirection: "row", alignItems: "center", gap: 9, marginBottom: 12 }, passengerIcon: { width: 30, height: 30, borderRadius: 10, backgroundColor: "#E6F0E8", alignItems: "center", justifyContent: "center" }, passengerName: { color: ink, fontWeight: "800" }, reservations: { gap: 8 }, reservation: { flexDirection: "row", alignItems: "center", gap: 12, padding: 11, borderRadius: 11, backgroundColor: "#F2F3EE" }, reservationTrain: { color: green, fontSize: 12, fontWeight: "800", flex: 1 }, reservationText: { color: "#747B76", fontSize: 12 },
